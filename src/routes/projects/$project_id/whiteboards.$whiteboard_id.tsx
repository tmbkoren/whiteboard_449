...
function RouteComponent() {
  ...
  const [messages, setMessages] = useState<Message[]>(
    (whiteboardData.chat_messages || []).map((m: any) => ({
      ...
      sender:
        m.sender ?? (m.sender_id === session?.user?.id ? 'user' : 'system'),
      senderName: m.senderName ?? m.sender_name ?? undefined,
      sent_at: m.timestamp ?? m.sent_at ?? undefined,
    }))
  );
          ...
          if (data.type === 'UPDATE_WHITEBOARD' && data.elements) {
            ...
            const normalized: Message = {
              ...
              senderName:
                raw.senderName ??
                raw.sender_username ??
                (raw.sender_id === session?.user?.id
                  ? (currentUsername ?? undefined)
                  : undefined),
              sent_at: raw.timestamp ?? raw.sent_at ?? undefined,
            };
